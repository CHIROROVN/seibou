...
class ProductModel
{
    ...
    public function getAll($where = array())
    {
        ...
        if ( isset($where['item_code_1']) && $where['item_code_1'] != '' && !empty($where['item_code_1']) ) {
          $db = $db->where($this->primaryId, 'LIKE', '%' . $where['item_code_1'] . '%');
        }
        if ( isset($where['item_code_2']) && $where['item_code_2'] != '' && !empty($where['item_code_2']) ) {
          
          $db = $db->orWhere($this->primaryId, 'LIKE', '%' . $where['item_code_2'] . '%');
        }
        if ( isset($where['item_code_3']) && $where['item_code_3'] != '' && !empty($where['item_code_3']) ) {
          
          $db = $db->orWhere($this->primaryId, 'LIKE', '%' . $where['item_code_3'] . '%');
        }
